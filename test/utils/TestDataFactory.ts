...
export class TestDataFactory {
  ...
  static createHost(overrides: Partial<any> = {}) {
    return {
      id: this.generateId(),
      email: faker.internet.email(),
      name: faker.person.fullName(),
  ...
  static createSecurity(overrides: Partial<any> = {}) {
    return this.createHost({
  ...
  static createInvitation(guestId: string, hostId: string, overrides: Partial<any> = {}) {
    const now = new Date()
  ...
  static createVisit(guestId: string, hostId: string, overrides: Partial<any> = {}) {
    const now = new Date()
  ...
  static createActiveVisit(guestId: string, hostId: string, overrides: Partial<any> = {}) {
  ...
  static createExpiredInvitation(guestId: string, hostId: string, overrides: Partial<any> = {}) {
  ...
  static createHostBatch(count: number, overrides: Partial<any> = {}): any[] {
    return Array.from({ length: count }, () => this.createHost(overrides))
  }

  static createScenario(type: 'atLimit' | 'overLimit' | 'underLimit', guest: any, host: any) {
    const visits = []
    const limits = {
      atLimit: 3,
      overLimit: 5,
