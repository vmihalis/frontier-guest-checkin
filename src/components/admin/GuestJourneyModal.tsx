                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            {/* Override Badge */}
                            {(event.data as { overrideReason?: string })?.overrideReason && (
                        ...
                        {event.type === 'terms_acceptance' && (event.data as { expiresAt?: string; isExpired?: boolean })?.expiresAt && (
                          <div className="mt-1">
                            <span className={`text-xs ${
                              (event.data as { isExpired?: boolean }).isExpired 
                                ? 'text-red-600 dark:text-red-400' 
                                : 'text-muted-foreground'
                            }`}>
                              {(event.data as { isExpired?: boolean; expiresAt?: string }).isExpired 
                                ? '⚠️ This acceptance has expired'
                                : `Valid until ${new Date((event.data as { expiresAt?: string }).expiresAt!).toLocaleDateString()}`}
                            </span>
                          </div>
                        )}
                        
                        {/* Additional Host Context */}
                        {(event.data as { invitationHost?: { name: string }; isHostMismatch?: boolean; hostName?: string })?.invitationHost && (event.data as { isHostMismatch?: boolean })?.isHostMismatch && (
                          <div className="mt-2 p-2 bg-gray-50 dark:bg-gray-800/50 rounded text-xs">
                            <div className="flex items-center gap-1 text-gray-600 dark:text-gray-400">
                              <Mail className="h-3 w-3" />
                              <span>Originally invited by: {(event.data as { invitationHost?: { name: string } }).invitationHost?.name}</span>
                            </div>
                            <div className="flex items-center gap-1 text-gray-600 dark:text-gray-400 mt-1">
                              <UserCheck className="h-3 w-3" />
                              <span>Visited: {(event.data as { hostName?: string }).hostName}</span>
                            </div>
                          </div>
                        )}
                      </div>
